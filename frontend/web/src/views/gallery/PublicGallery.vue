<template>
  <div class="public-gallery">
    <h2>作品展示</h2>
    <p class="hint">已上架模版封面，登录后可使用模版生图或充值</p>
    <div class="sort-bar">
    ...
    <el-row :gutter="12">
      <el-col v-for="item in items" :key="item.id" :xs="12" :sm="8" :md="6">
        <el-card :body-style="{ padding: '8px' }" shadow="hover">
          ...
          <p class="meta">使用 {{ item.useCount }} 次</p>
        </el-card>
      </el-col>
    </el-row>
    <el-pagination
...
export default {
  name: 'PublicGallery',
  data() {
    return { items: [], page: 1, size: 20, total: 0, sort: 'hot' }
  },
  mounted() {
    this.load()
  },
